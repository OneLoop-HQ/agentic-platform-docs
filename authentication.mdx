...
<Steps>
  <Step title="Send the creation request">
    ...
    curl -X POST https://api-sandbox.featherhq.com/v1/identity/api-keys \
      ...
      -d '{
        "name": "my-app-key",
        "scopes": ["conversations:read", "conversations:manage", "assistants:read"],
...
Feather applies a per-organization rate limit across the API — **6,000 requests per minute**. The limit is shared by every key in the organization, not per key.

When you exceed it, Feather returns `429 Too Many Requests`. Back off and retry after a short delay; batch or space out high-volume workloads to stay under the limit.

```json 429 Too Many Requests
{
  "error": "rate_limited",
  "message": "Organization rate limit exceeded. Retry after a short delay."
}
```
