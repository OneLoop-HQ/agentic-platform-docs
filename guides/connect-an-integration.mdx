---
title: "Connect Third-Party Integrations to Your Assistants"
sidebarTitle: "Connect Integration"
description: "Add Slack, Notion, Twilio, Google Drive, and custom MCP servers to Feather. Give your assistants real-world tools backed by your existing services."
---

Integrations extend your assistants beyond conversation — they let your AI act on the world by calling real APIs, sending messages, querying documents, and triggering workflows inside the tools your team already uses. Feather's integration layer handles OAuth flows, credential storage, and tool schema sync so you can focus on building assistant logic rather than plumbing.
...
<Steps>
  ...
  <Step title="Finalize the connection">
    ...
    Feather will immediately sync the tool schema for your new connection. Tools are ready to attach to assistants within seconds.
  </Step>
</Steps>
...
Prevent an assistant from calling specific tools without disconnecting the entire integration. The path takes the tool's `id` (the UUID from the connection's `tools` array), not its slug:
...
Before attaching a tool to an assistant, verify it works end-to-end using the test invoke endpoint.
...
The assistant will automatically receive the tool's input schema in its context and can invoke it during a conversation without any additional wiring.
