...
<Steps>
  ...
  <Step title="Compile the revision">
    Call `POST /v1/workflows/revisions/{revision_id}/compile`. The compiler parses your instructions and produces a `compiled_graph`. Choose a `compilation_strategy` of `fast`, `standard`, or `strong` to trade speed for thoroughness.

    Compilation is asynchronous — poll the revision's `compilation_status`.
  </Step>
